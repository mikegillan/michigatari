...
Each behind a one-function provider interface (the monetization seam; also how
tests inject fakes):
...
## 13. Licensing & Monetization Notes

- **AGPL-3.0** keeps the core open while requiring anyone hosting a modified
  version to publish their changes — the standard open-core choice.
- Potential future paid hosted tier (not built now, enabled by the provider
  seams and the pure-data project format): commercial tile/geocoding/routing
  providers, satellite and terrain styles, server-side render farm (ProRes,
  render queue), cloud projects/sharing/templates. Gating 4K export in the
  hosted tier is an option; the FOSS build has no gating.
- No AI attribution in commits; attribution noted in the README.
